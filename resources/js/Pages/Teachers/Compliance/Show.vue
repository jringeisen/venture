        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="bg-white shadow p-8 rounded-lg dark:bg-primary-gray">
                <div class="sm:flex sm:items-center sm:justify-between">
                    <div>
                        <h1 class="text-lg font-semibold text-gray-900 dark:text-neutral-300">{{ report.title }}</h1>
            ...
            <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
                <div class="rounded-lg bg-white p-4 shadow dark:bg-primary-gray">
                    <dt class="text-xs text-gray-500 dark:text-neutral-500">Instruction Days</dt>
                    <dd class="mt-1 text-2xl font-semibold text-gray-900 dark:text-neutral-300">{{ report.summary.total_instruction_days }}</dd>
                </div>
                <div class="rounded-lg bg-white p-4 shadow dark:bg-primary-gray">
                    <dt class="text-xs text-gray-500 dark:text-neutral-500">Instruction Hours</dt>
                    <dd class="mt-1 text-2xl font-semibold text-gray-900 dark:text-neutral-300">{{ report.summary.total_instruction_hours }}</dd>
                </div>
                <div class="rounded-lg bg-white p-4 shadow dark:bg-primary-gray">
                    <dt class="text-xs text-gray-500 dark:text-neutral-500">Active Courses</dt>
                    <dd class="mt-1 text-2xl font-semibold text-gray-900 dark:text-neutral-300">{{ report.summary.courses_active }}</dd>
                </div>
                <div class="rounded-lg bg-white p-4 shadow dark:bg-primary-gray">
                    <dt class="text-xs text-gray-500 dark:text-neutral-500">Completed</dt>
                    <dd class="mt-1 text-2xl font-semibold text-gray-900 dark:text-neutral-300">{{ report.summary.courses_completed }}</dd>
                </div>
                <div class="rounded-lg bg-white p-4 shadow dark:bg-primary-gray">
                    <dt class="text-xs text-gray-500 dark:text-neutral-500">Avg Trivia Score</dt>
                    <dd class="mt-1 text-2xl font-semibold text-gray-900 dark:text-neutral-300">{{ report.summary.average_trivia_score }}%</dd>
                </div>
                <div class="rounded-lg bg-white p-4 shadow dark:bg-primary-gray">
                    <dt class="text-xs text-gray-500 dark:text-neutral-500">Interactions</dt>
                    <dd class="mt-1 text-2xl font-semibold text-gray-900 dark:text-neutral-300">{{ report.summary.total_interactions }}</dd>
                </div>
            </div>
            ...
            <div class="bg-white shadow rounded-lg dark:bg-primary-gray">
                <button @click.prevent="toggleSection('activity')" class="flex w-full items-center justify-between p-6">
                    <h2 class="text-base font-semibold text-gray-900 dark:text-neutral-300">Daily Activity Log</h2>
                    <ChevronDownIcon :class="['h-5 w-5 text-gray-500 transition-transform', openSections.activity ? 'rotate-180' : '']"/>
                </button>
                        ...
                        <table class="min-w-full divide-y divide-gray-300 dark:divide-neutral-700">
                            ...
                            <tbody class="divide-y divide-gray-200 bg-white dark:bg-neutral-500 dark:divide-neutral-600">
                                <tr v-for="entry in activityLog" :key="entry.date">
                                    <td class="whitespace-nowrap py-4 pl-6 pr-3 text-sm text-gray-900 dark:text-primary-gray">{{ entry.date }}</td>
                                    <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500 dark:text-primary-gray">{{ formatDuration(entry.total_seconds) }}</td>
                                    <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500 dark:text-primary-gray">{{ entry.sessions }}</td>
                                    <td class="px-3 py-4 text-sm text-gray-500 dark:text-primary-gray">{{ entry.courses_studied.join(', ') || 'N/A' }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                    <p v-else class="p-6 text-sm text-gray-500 dark:text-neutral-400">No activity recorded for this period.</p>
                </div>
            </div>
            ...
            <div class="bg-white shadow rounded-lg dark:bg-primary-gray">
                ...
                <div v-if="openSections.courses" class="border-t border-gray-200 dark:border-neutral-600">
                    <div v-if="courseProgress.length > 0" class="divide-y divide-gray-200 dark:divide-neutral-600">
                        <div v-for="course in courseProgress" :key="course.title" class="p-6">
                            <div class="flex items-center justify-between">
                                <h3 class="text-sm font-semibold text-gray-900 dark:text-neutral-300">{{ course.title }}</h3>
                                <span class="text-sm text-gray-500 dark:text-neutral-400">{{ course.progress }}%</span>
                            </div>
                            ...
                            <div class="mt-2 flex gap-4 text-xs text-gray-500 dark:text-neutral-400">
                            ...
                            <div v-if="course.learning_objectives.length > 0" class="mt-3">
                                <p class="text-xs font-medium text-gray-700 dark:text-neutral-400">Learning Objectives:</p>
                                <ul class="mt-1 list-disc pl-5 text-xs text-gray-500 dark:text-neutral-400">
                    ...
                    <p v-else class="p-6 text-sm text-gray-500 dark:text-neutral-400">No course data for this period.</p>
                </div>
            </div>
            ...
            <div class="bg-white shadow rounded-lg dark:bg-primary-gray">
                <button @click.prevent="toggleSection('assessments')" class="flex w-full items-center justify-between p-6">
                    <h2 class="text-base font-semibold text-gray-900 dark:text-neutral-300">Assessment Results</h2>
                    <ChevronDownIcon :class="['h-5 w-5 text-gray-500 transition-transform', openSections.assessments ? 'rotate-180' : '']"/>
                </button>
                <div v-if="openSections.assessments" class="border-t border-gray-200 dark:border-neutral-600">
                    <div v-if="assessmentResults.length > 0" class="divide-y divide-gray-200 dark:divide-neutral-600">
                        <div v-for="assessment in assessmentResults" :key="assessment.course_title" class="p-6">
                            <div class="flex items-center justify-between">
                                <h3 class="text-sm font-semibold text-gray-900 dark:text-neutral-300">{{ assessment.course_title }}</h3>
                                <span class="text-sm font-medium text-gray-700 dark:text-neutral-400">Avg: {{ assessment.average }}%</span>
                            </div>
                    ...
                    <p v-else class="p-6 text-sm text-gray-500 dark:text-neutral-400">No assessment data for this period.</p>
                </div>
            </div>
            ...
            <div class="bg-white shadow rounded-lg dark:bg-primary-gray">
                <button @click.prevent="toggleSection('interactions')" class="flex w-full items-center justify-between p-6">
                    <h2 class="text-base font-semibold text-gray-900 dark:text-neutral-300">Student Interactions</h2>
                    <ChevronDownIcon :class="['h-5 w-5 text-gray-500 transition-transform', openSections.interactions ? 'rotate-180' : '']"/>
                </button>
                    ...
                    <div v-if="interactionLog.length > 0" class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-300 dark:divide-neutral-700">
                            <thead class="bg-gray-50 dark:bg-neutral-600">
                                <tr>
                                    <th scope="col" class="py-3.5 pl-6 pr-3 text-left text-sm font-semibold text-gray-900 dark:text-neutral-300">Date</th>
                                    <th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900 dark:text-neutral-300">Question</th>
                                    <th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900 dark:text-neutral-300">Word Count</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-200 bg-white dark:bg-neutral-500 dark:divide-neutral-600">
                                <tr v-for="(interaction, i) in interactionLog" :key="i">
                                    <td class="whitespace-nowrap py-4 pl-6 pr-3 text-sm text-gray-900 dark:text-primary-gray">{{ interaction.date }}</td>
                                    <td class="px-3 py-4 text-sm text-gray-500 dark:text-primary-gray max-w-md truncate">{{ interaction.question }}</td>
                                    <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500 dark:text-primary-gray">{{ interaction.word_count }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                    <p v-else class="p-6 text-sm text-gray-500 dark:text-neutral-400">No interactions recorded for this period.</p>
                </div>
            </div>
        </div>
    </div>
