    ...
    <div>
        <div v-if="loading || processing" role="status" class="max-w-sm">
            ...
            <div class="flex items-center mb-2">
                ...
                <span class="text-sm dark:text-neutral-400">Finding subject...</span>
            </div>
            <!-- Skeleton Loaders -->
        ...
        <div v-else class="flex items-center divide-x-4 divide-beach-teal">
            <div class="flex items-center p-2 pl-0">
                <ApplicationLogo class="w-14 h-14" />
                <h1 class="text-3xl font-bold dark:text-neutral-200">{{ subject?.toUpperCase() }}</h1>
            </div>
            <div v-if="subCategory" class="p-2">
                <h3 class="text-lg font-bold text-black tracking-widest dark:text-neutral-200">{{ subCategory.toUpperCase() }}</h3>
            </div>
        </div>
    </div>
</template>
