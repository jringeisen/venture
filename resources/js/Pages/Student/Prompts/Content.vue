        ...
        <div>
            ...
            <div v-if="!hasContent && connectionStatus !== 'error'" role="status">
                <div class="flex items-center mb-3">
                    ...
                    <span class="text-sm dark:text-neutral-500">{{ statusMessage }}</span>
                </div>
...
const setupEchoListener = () => {
    ...
    echoChannel = window.Echo.private(`prompts.${userId}`);

    echoChannel.listen('.reasoning_start', () => {
        isReasoning.value = true;
    });

    echoChannel.listen('.reasoning_end', () => {
        isReasoning.value = false;
    });

    echoChannel.listen('.text_delta', (event) => {
        if (event.delta) {
            message.value += event.delta;
        }
    });

    echoChannel.listen('.stream_end', () => {
...
const teardownEchoListener = () => {
    if (echoChannel) {
        echoChannel.stopListening('.reasoning_start');
        echoChannel.stopListening('.reasoning_end');
        echoChannel.stopListening('.text_delta');
        echoChannel.stopListening('.stream_end');
        window.Echo?.leave(`prompts.${userId}`);
        echoChannel = null;
    }
};
...
const startGeneration = () => {
    ...
    window.axios.post('/student/prompts/content')
        .then(() => {
