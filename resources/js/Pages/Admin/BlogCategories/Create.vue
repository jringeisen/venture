<template>
    <AdminLayout title="Create Category">
        <div class="max-w-2xl">
            <div class="mb-6">
                <Link :href="route('admin.blog-categories.index')" class="text-sm text-beach-text-light hover:text-beach-text dark:text-gray-400 dark:hover:text-gray-200">
            ...
            <div class="bg-white dark:bg-neutral-800 shadow-sm border border-slate-100 dark:border-neutral-700 rounded-lg">
                <div class="px-4 py-5 sm:p-6">
                    <h3 class="text-lg font-medium text-beach-text dark:text-white mb-6">Create New Category</h3>

                    <Form v-model="formData" :action="route('admin.blog-categories.store')" method="post" class="space-y-6" #default="{ errors, processing }">
                        <div>
                            <InputLabel for="name" value="Name"/>
                            <TextInput id="name" v-model="formData.name" name="name" type="text" class="mt-1 block w-full" required/>
                            <InputError :message="errors.name" class="mt-2"/>
                        </div>

                        <div>
                            <InputLabel for="description" value="Description"/>
                            <TextareaInput id="description" v-model="formData.description" name="description" class="mt-1 block w-full" rows="3"/>
